...
def get_model_form():
    ...
    with st.form(key='dimentia_form',clear_on_submit=True): 
        ...
        if submitted:
            ...
            form_list_values = []
            form_list_values.append(cerad)
            form_list_values.append(braak)
            form_list_values.append(ihc_tau2_ffpe)
            form_list_values.append(ihc_at8_ffpe)
            form_list_values.append(ihc_at8)
            form_list_values.append(ihc_a_beta_ffpe)
            form_list_values.append(ihc_a_beta)
            form_list_values.append(ihc_gfap_ffpe)
            form_list_values.append(ptau_ng_per_mg)
            form_list_values.append(vegf_pg_per_mg)
            form_list_values.append(ab42_over_ab40_ratio)
            form_list_values.append(ptau_over_tau_ratio)
